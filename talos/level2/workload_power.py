...
def validate_workload_aware_exploration(
    ...
    if profile is None:
        raise ValueError(
            f"{WORKLOAD_REQUIREMENTS_ERROR} Activity profile is missing."
        )
    if dram_ip.power_model is None:
        raise ValueError(
            f"{WORKLOAD_REQUIREMENTS_ERROR} DRAM IP {dram_ip.id!r} has no "
            "power_model."
        )
    ...
    if len(pe_counts) != 1:
        raise ValueError(
            f"{WORKLOAD_REQUIREMENTS_ERROR} Exactly one PE component is required."
        )
    for layer in profile.layers:
...
def _dram_power(ip: IPBlock, layer: LayerActivity) -> float:
    ...
    if model is None:
        raise ValueError(
            f"missing_characterization: DRAM IP {ip.id!r} has no power_model."
        )
    utilization = _memory_utilization(
...
def _validate_utilization(value: float, label: str) -> float:
    ...
    if value > 1.0 + UTILIZATION_TOLERANCE:
        code = (
            "insufficient_dram_bandwidth"
            if "dram" in label.lower()
            else "insufficient_memory_bandwidth"
        )
        raise ValueError(f"{code}: {label} exceeds 1: {value}.")
...
def _validate_selected_characterizations(
    ...
    if dram_ip.power_model is None:
        raise ValueError(
            f"missing_characterization: DRAM IP {dram_ip.id!r} has no "
            "power_model."
        )
